...
echo -n "   Entrez l'IP LAN du serveur [192.168.10.139] : "
read -r SERVER_IP
SERVER_IP="${SERVER_IP:-192.168.10.139}"
info "IP serveur : $SERVER_IP"

# Remplacer l'IP dans api.ts et socket.ts
sed -i "s|https://192.168.10.139/api|https://${SERVER_IP}/api|g" src/services/api.ts
sed -i "s|https://192.168.10.139|https://${SERVER_IP}|g" src/services/socket.ts
sed -i "s|192.168.10.139|${SERVER_IP}|g" android/app/src/main/res/xml/network_security_config.xml
